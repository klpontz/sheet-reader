...
Spreadsheets organize long answers well. They read badly. When a cell holds a
paragraph, the grid truncates it, and reading a batch of form responses turns
into squinting at a row of clipped text.

This is one HTML file that renders each row as a readable card.
...
Real spreadsheet exports are messier than they look, so it deals with commas
and line breaks inside quoted cells, blank column headers, duplicate column
names, right-to-left text, and long unbroken strings like message IDs. Type in
...
Colors and type follow [kevinrocci.com](https://kevinrocci.com).
